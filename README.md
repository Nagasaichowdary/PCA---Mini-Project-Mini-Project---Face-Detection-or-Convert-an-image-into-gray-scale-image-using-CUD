...
## Procedure:
1.Load the input image using the stb_image library. 2.Allocate memory on the GPU for the input and output image buffers. 3.Copy the input image data from the CPU to the GPU. 4.Define a CUDA kernel function that performs the grayscale conversion on each pixel of the image. 5.Launch the CUDA kernel with appropriate grid and block dimensions. 6.Copy the resulting grayscale image data from the GPU back to the CPU. 7.Save the grayscale image using the stb_image_write library. 8.Clean up allocated memory.
...
## OUTPUT :
## INPUT IMAGE:
![image](https://github.com/user-attachments/assets/209e0b87-6cc0-417b-b465-fcb5540b7b4b)
## Grayscale Image:
![image](https://github.com/user-attachments/assets/6e9fce48-8cdf-4eb9-9fcf-01f574f2b45a)
## Result:
